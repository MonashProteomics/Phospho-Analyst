...
server <- function(input, output,session){
  ...
  cleaned_data<- reactive({
    ...
    if (any(grep("PTM.Quantity", colnames(phospho_data())))){ # output from spectronaut
      ...
      if (any(grepl("^CON__|^Cont_",phospho_data()$PTM.ProteinId))){
        filtered_data <- phospho_data() %>% dplyr::filter(!grepl("^CON__|^Cont_", PTM.ProteinId))
      } else {
  ...
  pca_input<-reactive({ 
    ...
    if (num_total()<=500){
      if(length(levels(as.factor(colData(dep())$replicate))) <= 6){
        pca_plot<-plot_pca_new(dep(), n=num_total(), point_size = 4, label = input$pca_label)
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
      else{
        pca_plot<-plot_pca_new(dep(), n=num_total(), point_size = 4, indicate = "condition", label = input$pca_label) 
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
    }else{
      if(length(levels(as.factor(colData(dep())$replicate))) <= 6){
        pca_plot<-plot_pca_new(dep(), point_size = 4, label = input$pca_label)
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
      else{
        pca_plot<-plot_pca_new(dep(), point_size = 4, indicate = "condition", label = input$pca_label)
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
    }
  ...
  abundance_cntrst <- reactive({
    input$abundance_cntrst
  })
  
  save_plot_server("abundance_rank", saved_abundance_rank)
  save_plot_server("abundance_comp", saved_abundance_comp, abundance_cntrst)
  ...
  save_plot_server("pca", pca_input)
  save_plot_server("correlation", correlation_input)
  save_plot_server("cvs", cvs_input)
  save_plot_server("numbers", numbers_input)
  save_plot_server("coverage", coverage_input)
  save_plot_server("normalization", norm_input)
  save_plot_server("missing-values", missval_input)
  save_plot_server("imputation", imputation_input)
  ...
  ## PCA Plot 
  pca_input_pr<-reactive({
    if(input$analyze==0 ){
      return()
    }
    if (num_total_pr()<=500){
      if(length(levels(as.factor(colData(dep_pr())$replicate))) <= 6){
        pca_plot<- plot_pca_new(dep_pr(), n=num_total_pr(), point_size = 4, label = input$pca_label_pr)
        pca_plot<-pca_plot + labs(title = "PCA plot")
        return(pca_plot)
      }
      else{
        pca_plot<-plot_pca_new(dep_pr(), n=num_total_pr(), point_size = 4, indicate = "condition", label = input$pca_label_pr)
        pca_plot<-pca_plot + labs(title = "PCA plot")
        return(pca_plot)
      }
    }
      ...
      if(length(levels(as.factor(colData(dep_pr())$replicate))) <= 6){
        pca_plot<-plot_pca_new(dep_pr(), point_size = 4, label = input$pca_label_pr)
        pca_plot<-pca_plot + labs(title = "PCA plot")
        return(pca_plot)
      }else{
        pca_plot<-plot_pca_new(dep_pr(), point_size = 4, indicate = "condition", label = input$pca_label_pr)
        pca_plot<-pca_plot + labs(title = "PCA plot")
        return(pca_plot)
      }
    }
  ...
  save_plot_server("heatmap_pr", heatmap_input_pr)
  save_plot_server("volcano_pr", saved_volcano_pr, volcano_cntrst_pr)
  save_plot_server("protein_plot_pr", protein_input_pr,type_pr)
  ...
  save_plot_server("pca_pr", pca_input_pr)
  save_plot_server("correlation_pr", correlation_input_pr)
  save_plot_server("cvs_pr", cvs_input_pr)
  save_plot_server("numbers_pr", numbers_input_pr)
  save_plot_server("coverage_pr", coverage_input_pr)
  save_plot_server("normalization_pr", norm_input_pr)
  save_plot_server("missing-values_pr", missval_input_pr)
  save_plot_server("imputation_pr", imputation_input_pr)
...
#   )
  ...
  # scatter plot download
  volcano_comp <- reactive({
    input$volcano_comp
  })
  save_plot_server("scatter_plot", scatter_plot, volcano_comp)
  
  # QC plots
  save_plot_server("pca_plot_c",pca_input_c)
  save_plot_server("Phospho-correlation", correlation_input)
  save_plot_server("Protein-correlation", correlation_input_pr)
  save_plot_server("cvs_c", sample_cvs_input_c)
  save_plot_server("numbers_c", numbers_input_c)
  save_plot_server("coverage_c", coverage_input_c)
  save_plot_server("normalization_c", norm_input_c)
  save_plot_server("Phospho-missing-values", missval_input)
  save_plot_server("Protein-missing-values", missval_input_pr)
  save_plot_server("imputation_c", imputation_input_c)
  
  #  side-by-side interactive plots
  save_plot_server("interaction_comp",combined_inter)
  save_plot_server("bubble_comp",combined_point)
  ...
  ## PCA Plot
  pca_input_nr<-reactive({ 
    if(input$analyze==0 ){
      return()
    }
    if (num_total_nr()<=500){
      if(length(levels(as.factor(colData(dep_nr())$replicate))) <= 6){
        pca_plot<-plot_pca_new(dep_nr(), n=num_total_nr(), point_size = 4, label = input$pca_label_nr)
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
      else{
        pca_plot<-plot_pca_new(dep_nr(), n=num_total_nr(), point_size = 4, indicate = "condition", label = input$pca_label_nr) 
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
    }
      ...
      if(length(levels(as.factor(colData(dep_nr())$replicate))) <= 6){
        pca_plot<-plot_pca_new(dep_nr(),point_size = 4, label = input$pca_label_nr)
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
      else{
        pca_plot<-plot_pca_new(dep_nr(), point_size = 4, indicate = "condition", label = input$pca_label_nr)
        pca_plot<-pca_plot + labs(title = "PCA Plot")
        return(pca_plot)
      }
    }
  ...
  ### === Results Plot Download ==== ####
  saved_volcano_nr <- reactive({
    if(is.null(input$contents_nr_rows_selected) & is.null(input$protein_brush_nr)){
      p <- volcano_input_nr()
    }
    else{
  ...
  save_plot_server("heatmap_nr", heatmap_input_nr)
  save_plot_server("volcano_nr", saved_volcano_nr, volcano_cntrst_nr)
  save_plot_server("protein_plot_nr", protein_input_nr,type_nr)
  ...
  #   }
  # )
  save_plot_server("pca_plot_nr", pca_input_nr)
  save_plot_server("correlation_nr", correlation_input_nr)
  save_plot_server("cvs_nr", cvs_input_nr)
  save_plot_server("normalization_nr", norm_input_nr)
  save_plot_server("imputation_nr", imputation_input_nr)
  ...
  pca_input_dm<-reactive({
    if(length(levels(as.factor(colData(dep_dm())$replicate))) <= 6){
      pca_plot<-plot_pca_new(dep_dm(), point_size = 4, label = input$pca_label_dm)
      pca_plot<-pca_plot + labs(title = "PCA plot")
      return(pca_plot)
    }else{
      pca_plot<-plot_pca_new(dep_dm(), point_size = 4, indicate = "condition", label = input$pca_label_dm)
      pca_plot<-pca_plot + labs(title = "PCA plot")
      return(pca_plot)
    }
  })
  ...
  save_plot_server("numbers_dm", numbers_input_dm)
  save_plot_server("coverage_dm", coverage_input_dm)
  save_plot_server("normalization_dm", norm_input_dm)
  save_plot_server("missing-values_dm", missval_input_dm)
  ...
  pca_input_dm_pr<-reactive({
    if(length(levels(as.factor(colData(dep_dm_pr())$replicate))) <= 6){
      pca_plot<-plot_pca_new(dep_dm_pr(), point_size = 4, label = input$pca_label_dm_pr)
      pca_plot<-pca_plot + labs(title = "PCA plot")
      return(pca_plot)
    }else{
      pca_plot<-plot_pca_new(dep_dm_pr(), point_size = 4, indicate = "condition", label = input$pca_label_dm_pr)
      pca_plot<-pca_plot + labs(title = "PCA plot")
      return(pca_plot)
    }
  })
  ...
  save_plot_server("heatmap_dm_pr", heatmap_input_dm_pr)
  save_plot_server("volcano_dm_pr", saved_volcano_dm_pr, volcano_cntrst_dm_pr)
  save_plot_server("protein_plot_dm_pr", protein_input_dm_pr,type_dm_pr)
  ...
  ###### ==== DOWNLOAD QC plots (demo proteinGroup)==== ####
  ...
  #   }
  # )
  save_plot_server("pca_dm_pr", pca_input_dm_pr)
  save_plot_server("correlation_dm_pr", correlation_input_dm_pr)
  save_plot_server("cvs_dm_pr", cvs_input_dm_pr)
  save_plot_server("numbers_dm_pr", numbers_input_dm_pr)
  save_plot_server("coverage_dm_pr", coverage_input_dm_pr)
  save_plot_server("normalization_dm_pr", norm_input_dm_pr)
  save_plot_server("missing-values_dm_pr", missval_input_dm_pr)
  save_plot_server("imputation_dm_pr", imputation_input_dm_pr)
  ...
  ###### ==== DOWNLOAD plots (demo Comparison) ==== ####
  ...
  # )
  # scatter plot download
  volcano_comp_dm <- reactive({
    input$volcano_comp_dm
  })
  save_plot_server("scatter_plot_dm", scatter_plot_dm, volcano_comp_dm)
  
  # QC plots
  save_plot_server("pca_plot_c_dm",pca_input_c_dm)
  save_plot_server("Phospho-correlation_dm", correlation_input_dm)
  save_plot_server("Protein-correlation_dm", correlation_input_dm_pr)
  save_plot_server("cvs_c_dm", sample_cvs_input_c_dm)
  save_plot_server("numbers_c_dm", numbers_input_c_dm)
  save_plot_server("coverage_c_dm", coverage_input_c_dm)
  save_plot_server("normalization_c_dm", norm_input_c_dm)
  save_plot_server("Phospho-missing-values_dm", missval_input_dm)
  save_plot_server("Protein-missing-values_dm", missval_input_dm_pr)
  save_plot_server("imputation_c_dm", imputation_input_c_dm)
  
  #  side-by-side interactive plots
  save_plot_server("interaction_comp_dm",combined_inter_dm)
  save_plot_server("bubble_comp_dm",combined_point_dm)
  ...
  pca_input_dm_nr<-reactive({
    if(length(levels(as.factor(colData(dep_dm_nr())$replicate))) <= 6){
      pca_plot<-plot_pca_new(dep_dm_nr(),point_size = 4, label = input$pca_label_dm_nr)
      pca_plot<-pca_plot + labs(title = "PCA Plot")
      return(pca_plot)
    }
    else{
      pca_plot<-plot_pca_new(dep_dm_nr(), point_size = 4, indicate = "condition", label = input$pca_label_dm_nr)
      pca_plot<-pca_plot + labs(title = "PCA Plot")
      return(pca_plot)
    }
  })
  ...
  save_plot_server("heatmap_dm_nr", heatmap_input_dm_nr)
  save_plot_server("volcano_dm_nr", saved_volcano_dm_nr, volcano_cntrst_dm_nr)
  save_plot_server("protein_plot_dm_nr", protein_input_dm_nr,type_dm_nr)
  ...
  ###### ==== DOWNLOAD QC plots (demo phosphosite_corrected)==== ####
  ...
  #   }
  # )
  save_plot_server("pca_plot_dm_nr", pca_input_dm_nr)
  save_plot_server("correlation_dm_nr", correlation_input_dm_nr)
  save_plot_server("cvs_dm_nr", cvs_input_dm_nr)
  save_plot_server("normalization_dm_nr", norm_input_dm_nr)
  save_plot_server("imputation_dm_nr", imputation_input_dm_nr)
