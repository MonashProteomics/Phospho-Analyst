...
get_results_phospho <- function(dep, apply_anova = FALSE) {
  # Show error if inputs are not the required classes
  assertthat::assert_that(inherits(dep, "SummarizedExperiment"),
  ...
  if (apply_anova){
    ...
    ids$anova_p.val <- signif(ids$anova_p.val, digits = 6)
    ids$anova_p.adj <- signif(ids$anova_p.adj, digits = 6)
  } else {
...
get_results_proteins <- function(dep, apply_anova = FALSE) {
  # Show error if inputs are not the required classes
  assertthat::assert_that(inherits(dep, "SummarizedExperiment"),
                          is.logical(apply_anova),
                          length(apply_anova) == 1)
